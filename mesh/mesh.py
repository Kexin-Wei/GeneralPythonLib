...
from .plot_utils import ax3d_handle
...
class Mesh:
    def __init__(self):
        ...
        self.tri_ele = (
            np.array([1, 3, 2, 1, 2, 4, 1, 4, 3, 2, 3, 4]).reshape(-1, 3) - 1
        )  # in outer normal vector order
        self.top_vtx = np.array([4, 3, 2, 1]) - 1
        self.tri_line_comb = np.array([1, 2, 1, 3, 2, 3]).reshape(-1, 2) - 1
        self.tet_line_comb = (
            np.array([1, 2, 1, 3, 1, 4, 2, 3, 2, 4, 3, 4]).reshape(-1, 2) - 1
        )
    ...
    def read_from_meshio(self, mesh_file, **kwargs):
        mesh = meshio.read(str(mesh_file), **kwargs)
        self.vertices = mesh.points
        self.tri_elements = mesh.cells_dict["triangle"]
        if "tetra" in mesh.cell_data_dict:
            self.tet_elements = mesh.cell_data_dict["tetra"]
        else:
            self.tet_elements = None
    ...
    def read_mesh_json(self, mesh_filename):
        mesh_file = os.path.join(
            os.path.dirname(os.path.abspath(__file__)), mesh_filename
        )
    ...
    def _update_tri_elements(self):
        ...
        self.tri_set = self.tet_elements[:, self.tri_ele].reshape(
            -1, 3
        )  # all the triangle, has repeating triangle
        temp = [tuple(row) for row in np.sort(self.tri_set, axis=1)]
        temp2, indices, counts = np.unique(
            temp, axis=0, return_index=True, return_counts=True
        )
        self.tri_elements = self.tri_set[indices[np.argwhere(counts == 1)]].reshape(
            -1, 3
        )
    ...
    def _update_tri_tet(self):
        ...
        for tri_ele in self.tri_elements:
            match_tet = np.argwhere(
                (self.tri_set == tuple(tri_ele)).all(axis=1)
            ).squeeze()
            tri_tet[i] = np.r_[
                match_tet // 4, match_tet % 4, self.top_vtx[match_tet % 4]
            ].astype(
                int
            )  # ith tet elements, which face in tri_ele order
            i += 1
    ...
    def _update_tri_normal_vecs(self, vertices):
        ...
        tri_normal_vec = np.cross(
            tri_vtx[:, 1] - tri_vtx[:, 0], tri_vtx[:, 2] - tri_vtx[:, 0]
        )
        self.tri_normal_vec = (
            tri_normal_vec
            * (1.0 / np.linalg.norm(tri_normal_vec, axis=1))[:, np.newaxis]
        )
    ...
    def _update_vtx_normal_vecs(self):
        ...
        vtx_normal_vec = [
            self.tri_normal_vec[np.argwhere(self.tri_elements == iv)[:, 0]].sum(axis=0)
        ...
        self.vtx_normal_vec = (
            vtx_normal_vec
            * (1 / np.linalg.norm(vtx_normal_vec, axis=-1))[:, np.newaxis]
        )
    ...
    def _get_tet_line_segs(self):
        ...
        for i in range(self.tet_line_comb.shape[0]):
            tet_line_segs[i] = self.tet_elements[:, self.tet_line_comb[i]]
    ...
    def _get_tri_line_segs(self):
        ...
        for i in range(self.tri_line_comb.shape[0]):
            tri_line_segs[i] = self.tri_elements[:, self.tri_line_comb[i]]
    ...
    def update_normal_vecs(self):
        self._update_normal_vecs(self.vertices)
    ...
    def _plt_ps_normal_vecs(self, base_ps, nvs, cp="#7D75FE", cnv="#1D1788", **kwargs):
        ...
        if len(base_ps.shape) == 1:
            base_ps = base_ps.reshape(-1, 3)
        if len(nvs.shape) == 1:
            nvs = np.matlib.repmat(nvs, base_ps.shape[0], 1)
    ...
    def plt_tri_normal_vec(self, vtx_scl=1, vec_scl=1, **kwargs):
        ax = ax3d_handle(**kwargs)
        ax = self._plt_ps_normal_vecs(
            self.tri_mid * vtx_scl,
            self.tri_normal_vec * vec_scl,
            cp="#7D75FE",
            cnv="#1D1788",
            ax=ax,
    ...
    def _plt_fixed_vtx(self, vertices, fixed_vindex, **kwargs):
        ax = ax3d_handle(**kwargs)
        ax.scatter(
            vertices[fixed_vindex, 0],
            vertices[fixed_vindex, 1],
            vertices[fixed_vindex, 2],
            c="#780EB1",
        )  # purple
        return ax
    ...
    def set_axes_equal(ax: plt.Axes, limits=None):
        ...
        Mesh._set_axes_radius(ax, origin, radius)

    @staticmethod
    def plt_equal(ax, limits=None):
        ax.set_box_aspect((1, 1, 1))  # IMPORTANT - this is the new, key line
        Mesh.set_axes_equal(ax, limits=limits)  # IMPORTANT - this is also required

    def plot(self, ax=None, **kwargs):
        if ax is not None:
            ax.cla()
        ax = self.plot_vertices(ax=ax, **kwargs)
        self.plt_equal(ax)
        return ax

    def plt_x(self, draw_type="tri", text_opt="off", scl=1, **kwargs):
        ax = self._plt_msh(
            self.vertices * scl, draw_type=draw_type, text_opt=text_opt, **kwargs
        )
        if self.fixed_vindex is not None:
            ax = self._plt_fixed_vtx(self.vertices * scl, self.fixed_vindex, ax=ax)
        return ax

    def plot_vertices(self, draw_type="tri", text_opt="off", scl=1, **kwargs):
        ax = self._plt_msh(
            self.vertices * scl, draw_type=draw_type, text_opt=text_opt, **kwargs
        )
        return ax
    ...
    def _plt_msh(
        self, vertices, draw_type="tri", node_opt="on", text_opt="off", **kwargs
    ):
        ...
        if tet_opt == "on":  # blue
            if self.tet_line_segs is None:
                self._get_tet_line_segs()
            line_vt = np.hstack(
                (vertices[self.tet_line_segs[:, 0]], vertices[self.tet_line_segs[:, 1]])
            )
        ...
        if tri_opt == "on":  # green
            if self.tri_line_segs is None:
                self._get_tri_line_segs()
            line_vt = np.hstack(
                (vertices[self.tri_line_segs[:, 0]], vertices[self.tri_line_segs[:, 1]])
            ).copy()
        ...
        if node_opt == "on":
            if tri_opt == "on":  # orange
                if self.surf_vindex is None:
                    self.surf_vindex = np.unique(self.tri_elements)
                ax.scatter(
                    vertices[self.surf_vindex, 0],
                    vertices[self.surf_vindex, 1],
                    vertices[self.surf_vindex, 2],
                    marker="o",
                    c="#eb8c23",
                )
                if text_opt == "on":
                    for i in self.surf_vindex:
