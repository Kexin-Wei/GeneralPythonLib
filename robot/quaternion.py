...
class Quaternion:
    ...
    @staticmethod
    def pure(v: np.ndarray) -> "Quaternion":
...
class DualQuaternion:
    ...
    def from_real_dual(self, real: Quaternion, dual: Quaternion) -> "DualQuaternion":
    ...
    def vector(self) -> "DualQuaternion":
        return DualQuaternion(np.hstack([self.real.vector().v, self.dual.vector().v]))
    ...
    @staticmethod
    def pure_real(v: np.ndarray) -> "DualQuaternion":
        assert v.shape == (3,), "Vector must be a 3D vector."
    ...
    def from_euler_vector(
        ...
        Returns:
            DualQuaternion: _description_
        """
        q = Quaternion.from_euler(theta, n)
        # below  = from_quaternion_vector(q, v)
        t = Quaternion(np.hstack([0, v]))
        d = t * q * 0.5
        return DualQuaternion(np.hstack([q.v, d.v]))
